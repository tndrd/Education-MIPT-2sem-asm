...
{
    // refers to htable
    // operator returns this obj
    // proxy class
    // stores word to be hashed
    // get left of = ot right of it
    // rval: read
    // lval: write
    // may make return ref to empty elem
    // if want to add, add to empty (with no second search) then call add and create anpther empty
}
*/
...
hash_t opt11_crc32(const char* key)
{
    ...
    unsigned short* current_pair = (unsigned short*) key;
    unsigned short cpv = *current_pair;

    //printf("key: %s\n", key);

    while ((cpv & 0x00FF) && (cpv & 0xFF00))
    {
        //printf("  %c (%d) - %c (%d)\n");
        crc = _mm_crc32_u16(crc, cpv);
        cpv = *(++current_pair);
    }

    char* current_char = (char*)current_pair;
    
    char  ccv = *current_char;
    
    while (ccv)
    {
        crc = _mm_crc32_u8(crc, ccv);
        ccv = *(++current_char);
    }
